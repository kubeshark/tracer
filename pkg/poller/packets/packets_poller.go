...
func (p *PacketsPoller) poll() {
	...
	for chunk := range chunks {
		if err := p.handlePktChunk(chunk); err != nil {
			log.Error().Err(err).Msg("ERROR") //XXX
			utils.LogError(err)
		}
	}
}
...
func (p *PacketsPoller) handlePktChunk(chunk tracerPktChunk) error {
	...
	if ptr.Num != pkts.num {
		if ptr.ID == pkts.id {
			return fmt.Errorf("lost packet message(1) id: (%v %v) num: (%v %v) len: %v last: %v dir: %v tot_len: %v", pkts.id, ptr.ID, pkts.num, ptr.Num, ptr.Len, ptr.Last, ptr.Direction, ptr.TotLen)
		}
		...
		pkts.id = ptr.ID
		if ptr.Num != pkts.num {
			return fmt.Errorf("lost packet message(2) id: (%v %v) num: (%v %v) len: %v last: %v dir: %v tot_len: %v", pkts.id, ptr.ID, pkts.num, ptr.Num, ptr.Len, ptr.Last, ptr.Direction, ptr.TotLen)
		}
	}
	...
	if ptr.Last != 0 {
		...
		err := p.sorter.WritePlanePacket(0, ptr.CgroupID, ptr.Direction, layers.LayerTypeEthernet, p.ethhdr, gopacket.Payload(pkts.buf[:pkts.len]))
		if err != nil {
			return err
		}
...
func (p *PacketsPoller) pollChunksPerfBuffer(chunks chan<- tracerPktChunk) {
