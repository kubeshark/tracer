...
func FindContainerPids(containerIds map[string]v1.Pod) (map[uint32]v1.Pod, error) {
	...
	pids, err := os.ReadDir(Path)
	if err != nil {
		return result, err
	}

	log.Info().Str("procfs", Path).Int("pids", len(pids)).Msg("Starting TLS auto discoverer:")
...
func GetProcessCgroup(pid string) (string, error) {
	fpath := fmt.Sprintf("%s/%s/cgroup", Path, pid)

	bytes, err := os.ReadFile(fpath)
	if err != nil {
		return "", fmt.Errorf("Error reading cgroup file %s - %v", fpath, err)
	}
	...
	if cgrouppath == "" {
		return "", fmt.Errorf("Cgroup path not found for %s, %s", pid, lines)
	}
