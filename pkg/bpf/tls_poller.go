...
func NewTlsPoller(
	...
	poller := &TlsPoller{
		...
		gopacketWriter:     gopacketWriter,
		tai:                tai.NewTaiInfo(),
		reusableBuffer:     bytes.NewReader(nil),
	...
	// Prefer ringbuf when the map is a ringbuf map; otherwise fall back to perf.
	if rr, err := ringbuf.NewReader(perfBuffer); err == nil {
		poller.useRingbuf = true
		poller.ringReader = rr

		poller.reusableChunkBuf = make([]byte, int(unsafe.Sizeof(TracerTlsChunk{})))

		log.Info().Msg("TlsPoller: using ringbuf backend")
	} else {
		poller.chunksReader, err = perf.NewReader(perfBuffer, perfBufferSize)
		if err != nil {
			return nil, errors.Wrap(err, 0)
		}
		log.Info().Msg("TlsPoller: using perf backend")
	}
...
func (p *TlsPoller) Start() {
	...
	if p.useRingbuf {
		go p.pollChunksRingbufBuffer(chunks)
	} else {
...
// decodeTlsChunk pads a variable-size ringbuf sample into a full TracerTlsChunk,
// so binary.Read can safely decode the generated struct.
func (p *TlsPoller) decodeTlsChunk(raw []byte) (*TracerTlsChunk, error) {
	if len(raw) == 0 {
		return nil, fmt.Errorf("empty tls record")
	}

	need := int(unsafe.Sizeof(TracerTlsChunk{}))
	if p.reusableChunkBuf == nil || len(p.reusableChunkBuf) != need {
		p.reusableChunkBuf = make([]byte, need)
	}

	n := copy(p.reusableChunkBuf, raw)
	if n < need {
		clear(p.reusableChunkBuf[n:])
	}

	p.reusableBuffer.Reset(p.reusableChunkBuf)
	...
	// Defensive clamp (should already be done in BPF)
	if chunk.Recorded > uint32(len(chunk.Data)) {
		chunk.Recorded = uint32(len(chunk.Data))
	}

	return &chunk, nil
}

func (p *TlsPoller) pollChunksRingbufBuffer(chunks chan<- *TracerTlsChunk) {
	...
	for {
		...
		if err != nil {
			...
			log.Fatal().Err(err).Msg("Error reading tls records from ringbuf, aborting!")
			return
		}
		...
		chunk, derr := p.decodeTlsChunk(raw)
		if derr != nil {
			log.Error().Err(derr).Msg("Error parsing tls chunk")
			continue
		}
