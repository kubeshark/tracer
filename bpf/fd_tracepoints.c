...
fd_tracepoints_handle_openssl(void *ctx, __u32 fd, __u64 id,
                              struct ssl_info *infoPtr, void *map_fd,
                              __u64 origin_code) {
...
static __always_inline void fd_tracepoints_handle_go(void *ctx, __u32 fd,
                                                     __u64 id, void *map_fd,
                                                     __u64 origin_code) {
...
static __always_inline void handle_read(void *ctx, __u64 fd) {
  ...
  if (infoPtr != NULL) {
    fd_tracepoints_handle_openssl(ctx, fd, id, infoPtr, &openssl_read_context,
                                  ORIGIN_SYS_ENTER_READ_CODE);
  }

  fd_tracepoints_handle_go(ctx, fd, id, &go_kernel_read_context,
                           ORIGIN_SYS_ENTER_READ_CODE);
}
...
static __always_inline void handle_write(void *ctx, __u64 fd) {
  ...
  if (infoPtr != NULL) {
    fd_tracepoints_handle_openssl(ctx, fd, id, infoPtr, &openssl_write_context,
                                  ORIGIN_SYS_ENTER_WRITE_CODE);
  }

  fd_tracepoints_handle_go(ctx, fd, id, &go_kernel_write_context,
                           ORIGIN_SYS_ENTER_WRITE_CODE);
}
