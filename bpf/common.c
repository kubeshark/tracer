...
static __always_inline int add_address_to_chunk(struct pt_regs *ctx,
  ...
  // Happens when we don't catch the connect / accept (if the connection is
  // created before targeting is started)
  if (flags == NULL) {
    return 0;
  }
  ...
  if (info->address_info.family == AF_INET) {
    ...
    bpf_probe_read(&chunk->address_info.saddr4, sizeof(__be32),
                   &info->address_info.saddr4);
    bpf_probe_read(&chunk->address_info.daddr4, sizeof(__be32),
                   &info->address_info.daddr4);
  } else if (info->address_info.family == AF_INET6) {
    chunk->address_info.family = AF_INET6;
    bpf_probe_read(chunk->address_info.saddr6,
                   sizeof(chunk->address_info.saddr6),
                   info->address_info.saddr6);
    bpf_probe_read(chunk->address_info.daddr6,
                   sizeof(chunk->address_info.daddr6),
                   info->address_info.daddr6);
  } else {
...
static __always_inline int send_chunk_part(struct pt_regs *ctx,
                                           uintptr_t buffer, __u64 id,
                                           struct tls_chunk *chunk, int start,
                                           int end) {
  ...
  return bpf_perf_event_output(ctx, &chunks_buffer, BPF_F_CURRENT_CPU, chunk,
                               sizeof(struct tls_chunk));
}

static __always_inline int send_chunk(struct pt_regs *ctx, uintptr_t buffer,
                                      __u64 id, struct tls_chunk *chunk) {
  // ebpf loops must be bounded at compile time, we can't use (i < chunk->len /
  // CHUNK_SIZE)
  //
  // 	https://lwn.net/Articles/794934/
  //
  // However we want to run in kernel older than 5.3, hence we use "#pragma
  // unroll" anyway
  //
  int ret = 0;
#pragma unroll
...
static __always_inline void output_ssl_chunk(struct pt_regs *ctx,
                                             struct ssl_info *info,
                                             int count_bytes, __u64 id,
                                             __u32 flags, __u64 cgroup_id,
                                             struct save_stats *stats) {
  ...
  // If other thread, running on the same CPU get to this point at the same time
  // like us (context switch)
  ...
  if (!add_address_to_chunk(ctx, chunk, id, chunk->fd, info)) {
    // Without an address, we drop the chunk because there is not much to do
    // with it in Go
    //
    return;
  }
...
static __always_inline struct ssl_info new_ssl_info() {
  struct ssl_info info = {.fd = invalid_fd,
                          .created_at_nano = bpf_ktime_get_ns()};
  return info;
}

static __always_inline struct ssl_info
lookup_ssl_info(struct pt_regs *ctx, void *map_fd, __u64 pid_tgid) {
  ...
  if (infoPtr != NULL) {
    ...
    if ((bpf_ktime_get_ns() - info.created_at_nano) > SSL_INFO_MAX_TTL_NANO) {
      // If the ssl info is too old, we don't want to use its info because it
      // may be incorrect.
      //
      info.fd = invalid_fd;
