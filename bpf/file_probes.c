...
static long str_match_begin(const char *s1, __u32 sz1, const char *s2,
                            __u32 sz2) {
  if (sz1 < sz2)
    return 0;
...
static __always_inline void find_openssl(void *ctx, __u32 device_id, void *name,
                                         uint64_t cgroup_id, uint64_t inode,
                                         uint8_t remove) {
  __u32 zero = 0;
  ...
  for (int i = 0; i < 32; i++) {
    ...
    if (offset >= 0 && offset < MAX_FILEPATH) {
      if (str_match_begin(&p->path[offset], PATTERN_LIBSSL_LEN, PATTERN_LIBSSL,
                          PATTERN_LIBSSL_LEN)) {
        p->inode = inode;
        ...
        p->remove = remove;
        p->size = sz;
        bpf_perf_event_output(ctx, &perf_found_openssl, BPF_F_CURRENT_CPU, p,
                              sizeof(struct file_path));
      }
    }
  }
}
...
static __always_inline void find_cgroup_fs(void *ctx, const char *name) {
  ...
  char buf[CGROUPV1_FS_PATH_LEN +
           1]; // CGROUPV1_FS_PATH_LEN > CGROUPV2_FS_PATH_LEN
  int sz = 0;
...
int BPF_KPROBE(security_file_open) {
  ...
  void *file_path =
      get_path_str(__builtin_preserve_access_index(&file->f_path));
  ...
  if (flags & O_CREAT) {
    DEBUG_FILE_PROBE("SECURITY_FILE_OPEN: CREATE: %s, CGROUP: %lu", file_path,
                     cgroup_id);
    if (S_ISDIR(mode)) {
  ...
  DEBUG_FILE_PROBE("SECURITY_FILE_OPEN: inode: %lu filename: %s", inode,
                   file_path);
...
int BPF_KPROBE(security_inode_rename) {
  ...
  DEBUG_FILE_PROBE("RENAME OLD: dev: 0x%x inode: %lu filename: %s", old_dev,
                   old_ino, old_path);
  DEBUG_FILE_PROBE("RENAME NEW: dev: ox%x inode: %lu filename: %s", new_dev,
                   new_ino, new_path);
...
int BPF_KPROBE(security_inode_unlink) {
  ...
  DEBUG_FILE_PROBE("SECURITY_FILE_UNLINK: dev: %d inode: %lu filename: %s", dev,
                   ino, dentry_path);
...
int BPF_KPROBE(vfs_create) {
  ...
  __u64 dev = BPF_CORE_READ(dentry, d_inode, i_sb, s_dev);
  void *dentry_path = get_dentry_path_str(dentry);
  DEBUG_FILE_PROBE("VFS_CREATE: dev: %d inode: %lu filename: %s", dev, ino,
                   dentry_path);
...
int BPF_KPROBE(do_mkdirat_ret) {
  ...
  if (ret == 0) {
    bpf_perf_event_output(ctx, &perf_found_cgroup, BPF_F_CURRENT_CPU, p,
                          sizeof(struct file_path));
...
int BPF_KPROBE(security_path_mkdir) {
  ...
  __u64 cgroup_id = compat_get_current_cgroup_id(NULL);
  DEBUG_FILE_PROBE("SECURITY_PATH_MKDIR: dev: %d inode: %lu cgroup_id: %lu "
                   "path: %s filename: %s",
                   dev, ino, cgroup_id, path_str, dentry_path);
...
int cgroup_mkdir_signal(struct bpf_raw_tracepoint_args *ctx) {
  ...
  DEBUG_FILE_PROBE("cgroup_mkdir: h: %lu id: %llu path: %s", hierarchy_id,
                   cgroup_id, path);
  ...
  c->remove = 0;
  bpf_perf_event_output(ctx, &perf_cgroup_signal, BPF_F_CURRENT_CPU, c,
                        sizeof(struct cgroup_signal));
...
int cgroup_rmdir_signal(struct bpf_raw_tracepoint_args *ctx) {
  ...
  u32 hierarchy_id = BPF_CORE_READ(dst_cgrp, root, hierarchy_id);
  u64 cgroup_id = get_cgroup_id(dst_cgrp);
  DEBUG_FILE_PROBE("cgroup_rmdir: h: %lu id: %llu path: %s", hierarchy_id,
                   cgroup_id, path);
