...
typedef struct {
  ...
  __uint(max_entries,
         4096); // 800 KB    // simultaneous cgroup/skb ingress/eggress progs
  __type(key, indexer_t); // layer 3 header fields used as indexer
  __type(value, __u64);   // cgroup_id
} cgrpctxmap_t;
cgrpctxmap_t
    cgrpctxmap_in SEC(".maps"); // saved info SKB caller <=> SKB ingress
...
static __always_inline __u32 update_net_inodemap(struct socket *sock,
                                                 __u64 cgroup_id) {
...
int BPF_KPROBE(cgroup_bpf_run_filter_skb) {
  ...
  if (mightbecloned) {
    // pick network context from the sockmap (new sockptr <=> old inode <=>
    // task)
  ...
  void *data_ptr =
      BPF_CORE_READ(skb, head) + BPF_CORE_READ(skb, network_header);
  ...
  case 6:
    ...
    __builtin_memcpy(&indexer.src.in6_u, &nethdrs->iphdrs.ipv6hdr.saddr.in6_u,
                     4 * sizeof(u32));
    __builtin_memcpy(&indexer.dst.in6_u, &nethdrs->iphdrs.ipv6hdr.daddr.in6_u,
                     4 * sizeof(u32));
    break;
...
static __always_inline __u64 get_packet_cgroup(struct __sk_buff *ctx,
                                               void *cgrpctxmap) {
  __u64 cgroup_id = 0;
  switch (ctx->family) {
  case PF_INET:
