...
(ABI0) After 1.17, Go switched to an internal register-based calling convention.
(ABIInternal)

`uretprobe` in Linux kernel uses trampoline pattern to jump to original return
address of the probed function. A Goroutine's stack size is 2Kb while a C thread
is 2MB on Linux. If stack size exceeds 2Kb, Go runtime relocates the stack. That
causes the return address to become incorrect in case of `uretprobe` and probed
Go program crashes. Therefore `uretprobe` CAN'T BE USED for a Go program.

`_ex_uprobe` suffixed probes suppose to be `uretprobe`(s) are actually
`uprobe`(s) because of the non-standard ABI of Go. Therefore we probe all `ret`
mnemonics under the symbol by automatically finding them through reading the ELF
binary and disassembling the symbols. Disassembly related code located in
`go_offsets.go` file and it uses Capstone Engine. Solution based on:
https://github.com/iovisor/bcc/issues/1320#issuecomment-407927542 *Example* We
probe an arbitrary point in a function body (offset +559):
...
Tracing Go Functions with eBPF (<=1.16):
https://www.grant.pizza/blog/tracing-go-functions-with-ebpf-part-2/ Challenges
of BPF Tracing Go: https://blog.0x74696d.com/posts/challenges-of-bpf-tracing-go/
x86 calling conventions: https://en.wikipedia.org/wiki/X86_calling_conventions
Plan 9 from Bell Labs: https://en.wikipedia.org/wiki/Plan_9_from_Bell_Labs
The issue for calling convention change in Go:
https://github.com/golang/go/issues/40724 Go ABI0 (<=1.16) specification:
https://go.dev/doc/asm Proposal of Register-based Go calling convention:
https://go.googlesource.com/proposal/+/master/design/40724-register-calling.md
Go internal ABI (1.17) specification:
https://go.googlesource.com/go/+/refs/heads/dev.regabi/src/cmd/compile/internal-abi.md
Go internal ABI (current) specification:
https://go.googlesource.com/go/+/refs/heads/master/src/cmd/compile/abi-internal.md
A Quick Guide to Go's Assembler:
https://go.googlesource.com/go/+/refs/heads/dev.regabi/doc/asm.html Dissecting
Go Binaries: https://www.grant.pizza/blog/dissecting-go-binaries/ Capstone
Engine: https://www.capstone-engine.org/
*/
...
// TODO: cilium/ebpf does not support .kconfig Therefore; for now, we build
// object files per kernel version. Error: reference to .kconfig: not supported
...
get_goid_from_thread_local_storage(__u64 *goroutine_id) {
  int zero = 0;
  int one = 1;
  ...
  bpf_probe_read_user(&g_addr, sizeof(void *),
                      (void *)(fsbase + offsets->g_addr_offset));
  bpf_probe_read_user(goroutine_id, sizeof(void *),
                      (void *)(g_addr + offsets->goid_offset));
...
go_crypto_tls_get_fd_from_tcp_conn(struct pt_regs *ctx, enum ABI abi, __u32 pid,
                                   __u32 *fd) {
  struct go_interface conn;
  long err;
  __u64 addr;
#if defined(bpf_target_arm64)
  err = bpf_probe_read(&addr, sizeof(addr),
                       (void *)GO_ABI_INTERNAL_PT_REGS_SP(ctx) + 0x8);
  if (err != 0) {
    return err;
  }
  ...
  if (pi != NULL && pi->sys_fd_offset != -1) {
    ...
    err = bpf_probe_read(&net_conn_struct_ptr, sizeof(net_conn_struct_ptr),
                         conn.ptr);
    if (err != 0) {
      return err;
    }

    err = bpf_probe_read(fd, sizeof(*fd),
                         net_conn_struct_ptr + pi->sys_fd_offset);
    if (err != 0) {
      return err;
    }
  ...
  // if pids information is not descovered, supposing to find system file
  // descriptor at the offset 0x10
...
go_crypto_tls_uprobe(struct pt_regs *ctx, void *go_context, enum ABI abi) {
...
#if defined(bpf_target_arm64)
  err = bpf_probe_read(&info.buffer_len, sizeof(__u32),
                       (void *)GO_ABI_INTERNAL_PT_REGS_SP(ctx) + 0x18);
  if (err != 0) {
    log_error(ctx, LOG_ERROR_READING_BYTES_COUNT, pid_tgid, err,
              ORIGIN_SSL_UPROBE_CODE);
    return;
  }
#elif defined(bpf_target_x86)
  if (abi == ABI0) {
    err = bpf_probe_read(&info.buffer_len, sizeof(__u32),
                         (void *)GO_ABI_0_PT_REGS_SP(ctx) + 0x18);
    if (err != 0) {
      log_error(ctx, LOG_ERROR_READING_BYTES_COUNT, pid_tgid, err,
                ORIGIN_SSL_UPROBE_CODE);
      return;
...
#if defined(bpf_target_x86)
  if (abi == ABI0) {
    err = bpf_probe_read(&info.buffer, sizeof(__u32),
                         (void *)GO_ABI_0_PT_REGS_SP(ctx) + 0x11);
    if (err != 0) {
      log_error(ctx, LOG_ERROR_READING_FROM_SSL_BUFFER, pid_tgid, err,
                ORIGIN_SSL_UPROBE_CODE);
      return;
    }
...
static __always_inline void
go_crypto_tls_ex_uprobe(struct pt_regs *ctx, void *go_context,
                        void *go_user_kernel_context, __u32 flags,
                        enum ABI abi) {
  struct gotls_stats *stats = stats_gotls();
  if (stats == NULL) {
    return;
  ...
  if (err != 0) {
    log_error(ctx, LOG_ERROR_READING_SSL_CONTEXT, pid_tgid, err,
  ...
  if (flags == FLAGS_IS_READ_BIT) {
#if defined(bpf_target_arm64)
    // On ARM64 we look at a general-purpose register as an indicator of error
    // return
    ...
    if (abi == ABI0) {
      // n in return n, nil
      err = bpf_probe_read(&info.buffer_len, sizeof(__u32),
                           (void *)GO_ABI_0_PT_REGS_SP(ctx) + 0x28);
      if (err != 0) {
        log_error(ctx, LOG_ERROR_READING_BYTES_COUNT, pid_tgid, err,
                  ORIGIN_SSL_UPROBE_CODE);
        return;
...
#endif
    // This check achieves ignoring 0 length reads (the reads result with an
    // error)
  ...
  struct address_info *address_info =
      bpf_map_lookup_elem(go_user_kernel_context, &key);
  // Ideally we would delete the entry from the map after reading it,
  // but sometimes the uprobe is called twice in a row without the tcp kprobes
  // in between to fill in the entry again. Keeping it in the map and rely on
  // LRU logic.
  if (address_info == NULL) {
    log_error(ctx, LOG_ERROR_GETTING_GO_USER_KERNEL_CONTEXT, pid_tgid,
              info_ptr->fd, err);
    return;
  }
  ...
  if (address_info->family == AF_INET) {
    ...
    bpf_probe_read_kernel(info.address_info.daddr6,
                          sizeof(info.address_info.daddr6),
                          address_info->daddr6);
    bpf_probe_read_kernel(info.address_info.saddr6,
                          sizeof(info.address_info.saddr6),
                          address_info->saddr6);
...
int BPF_KPROBE(go_crypto_tls_abi0_write_ex) {
  go_crypto_tls_ex_uprobe(ctx, &go_write_context, &go_user_kernel_write_context,
                          0, ABI0);
  return 1;
}
...
int BPF_KPROBE(go_crypto_tls_abi0_read_ex) {
  go_crypto_tls_ex_uprobe(ctx, &go_read_context, &go_user_kernel_read_context,
                          FLAGS_IS_READ_BIT, ABI0);
  return 1;
}
