...
void sys_exit_accept4(struct sys_exit_accept4_ctx *ctx) {
  ...
  struct accept_info *infoPtr =
      bpf_map_lookup_elem(&accept_syscall_context, &id);
  ...
  if (err != 0) {
    log_error(ctx, LOG_ERROR_PUTTING_CONNECTION_CONTEXT, id, err,
              ORIGIN_SYS_EXIT_ACCEPT4_CODE);
  }
}
...
void sys_exit_connect(struct sys_exit_connect_ctx *ctx) {
  ...
  struct connect_info *infoPtr =
      bpf_map_lookup_elem(&connect_syscall_info, &id);
