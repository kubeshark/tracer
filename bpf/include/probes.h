...
enum buf_idx_e { STRING_BUF_IDX, FILE_BUF_IDX, MAX_BUFFERS };
...
// Read the file path to the given buffer, returning the start offset of the
// path.
...
#pragma unroll
  for (int i = 0; i < MAX_PATH_COMPONENTS; i++) {
    ...
    if (off <= buf_off) { // verify no wrap occurred
      ...
      sz = bpf_probe_read_kernel_str(
          &(out_buf->buf[off & ((MAX_PERCPU_BUFSIZE >> 1) - 1)]), len,
          (void *)d_name.name);
    } else
      break;
    if (sz > 1) {
      buf_off -= 1; // remove null byte termination with slash sign
      bpf_probe_read_kernel(&(out_buf->buf[buf_off & (MAX_PERCPU_BUFSIZE - 1)]),
                            1, &slash);
      buf_off -= sz - 1;
    } else {
  ...
  if (buf_off == (MAX_PERCPU_BUFSIZE >> 1)) {
    ...
    buf_off = 0;
    d_name = get_d_name_from_dentry(dentry);
    bpf_probe_read_kernel_str(&(out_buf->buf[0]), MAX_STRING_SIZE,
                              (void *)d_name.name);
  } else {
    // Add leading slash
    buf_off -= 1;
    ...
    bpf_probe_read_kernel(&(out_buf->buf[(MAX_PERCPU_BUFSIZE >> 1) - 1]), 1,
                          &zero);
  }
  return buf_off;
}
...
#pragma unroll
  for (int i = 0; i < MAX_PATH_COMPONENTS; i++) {
    ...
    if (off <= buf_off) { // verify no wrap occurred
      len = len & ((MAX_PERCPU_BUFSIZE >> 1) - 1);
      sz = bpf_probe_read_kernel_str(
          &(string_p->buf[off & ((MAX_PERCPU_BUFSIZE >> 1) - 1)]), len,
          (void *)d_name.name);
    } else
      break;
    if (sz > 1) {
      ...
      bpf_probe_read_kernel(
          &(string_p->buf[buf_off & (MAX_PERCPU_BUFSIZE - 1)]), 1, &slash);
      buf_off -= sz - 1;
    } else {
      // If sz is 0 or 1 we have an error (path can't be null nor an empty
      // string)
  ...
  if (buf_off == (MAX_PERCPU_BUFSIZE >> 1)) {
    ...
    buf_off = 0;
    struct qstr d_name = get_d_name_from_dentry(dentry);
    bpf_probe_read_kernel_str(&(string_p->buf[0]), MAX_STRING_SIZE,
                              (void *)d_name.name);
  } else {
    // Add leading slash
    buf_off -= 1;
