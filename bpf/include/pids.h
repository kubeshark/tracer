...
static __always_inline int is_task_from_netns(struct task_struct *task) {
  ...
  int level =
      BPF_CORE_READ(task, group_leader, nsproxy, pid_ns_for_children, level);

  for (int i = 0; i < TRACER_NAMESPACES_MAX; i++) {
    if ((level - i) < 0) {
      break;
    }
    inum = BPF_CORE_READ(task, group_leader, thread_pid, numbers[level - i].ns,
                         ns.inum);
...
tracer_get_task_pid_tgid(__u64 base_pid_tgid, struct task_struct *task) {
  unsigned int inum;
  ...
  for (int i = 0; i < TRACER_NAMESPACES_MAX; i++) {
    ...
    if (inum == TRACER_NS_INO) {
      __u64 ret =
          BPF_CORE_READ(task, group_leader, thread_pid, numbers[level - i].nr);
